...
export default function LightDirection({parentCallback}){
    const [selected, setSelected] = useState(0) 
    const [enabled, setEnabled] = useState(false) 

    const config = Config.photographySettings.definedDirection; 
                                ...
                                enabled?(
                                    ...
                                    <Radio.Group onChange={onChange} defaultValue={0} disabled={!enabled}>
                                        <Space direction="vertical">
                                        {
                                            config.map((el, indx) => {
                                                return(
                                                <Radio value={indx} key={indx}><b>{el.name}:</b> {el.description}</Radio>
                                                )
                                            })
                                        }
                                        </Space>
